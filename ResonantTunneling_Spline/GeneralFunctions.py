...
import math 
from   Parameters_Splinev1 import *
import matplotlib.pyplot as plt
import pickle


#-------------------------------------------------------------
# get value of potential on the array/grid
#-------------------------------------------------------------
...
def callNumerov(E,xp,n,ynm1,yn):
    Ni = 0 # first iteration in x-xp
    #K =  0.38929     # force constant From Mathematica Fit
    h = (xp)/n         # grid size from -xp to xp
    m = 1.0       # reduced mass of Hydrogen molecule in atomic unit
# starting points
    ...
    g_n_minus_1    =  2*m*(E-fittedE[0]) #2*m*(E-Morse(x,D0,a0,Req0))
    g_n            =  2*m*(E-fittedE[1]) #2*m*(E-Morse(x+h,D0,a0,Req0))
    g_n_plus_1     =  2*m*(E-fittedE[2]) #2*m*(E-Morse(x+2*h,D0,a0,Req0))
...
#   print("ll",y_n_plus_1)
    while  x < xp :     # Numerov method loop
        g_n_minus_1 ;
        g_n         ;
        g_n_plus_1     =  2*m*(E-fittedE[Ni]) #2*m*(E-Morse(x+2*h,D0,a0,Req0))
        y_n_plus_1     = (2*y_n*(1 - (5*h**2/12)*g_n) - y_n_minus_1*(1 + (h**2/12)*g_n_minus_1)) / (1 + (h**2/12)*g_n_plus_1)
        y.append(y_n_plus_1) # Solution array, unnormalized
        xarray.append(x+2*h) # corresponding x values
        x=x+h
        ...
        Ni = Ni + 1
#        if(Ni==12000):
#            print("g_n_plus_1,m,E,x,fittE(x+2*h)) y[-1]",g_n_plus_1,m,E,x,fittedE[Ni-1],y[-1])
    return xarray,y
...
def findEigenFunctions(Emin, Emax):
    ...
    # New variable for exiting IInd Bisection
    IIBisec = 0 # if IIBisec =  1 go for second bisection
...
# or [2] region with real root. For Possibility [1] Lowest decreasing part is found for root search, if f(x) are having same sign
    ...
    while(secondbs < 100):          # Hopefully 50 bisection is enough !
        ...
        if(abs(Emid-Emin)< 2e-15): # Crucial step ~ Machine Precision, used 2e-16
...
# if we want to save PNG files
            plt.plot(xo[0:grid-gridShift],yo[0:grid-gridShift], linestyle='--', marker='o', markersize=1,color='g')
            print(Emid-Emin)
    ...
    zeroP = [0] * gridShift
    return         0,xo[0:grid],yo[0:grid-gridShift]+zeroP # E=0 means exception occured
