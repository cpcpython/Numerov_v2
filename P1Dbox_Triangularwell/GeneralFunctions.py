...
def callNumerov(E,xp,n,ynm1,yn):
    ...
    m = 1.             # mass of electron in atomic unit
# starting points
    y_n_minus_1= ynm1 # first y
    y_n        = yn   # second y
    y          =[]    # third y, the calculated point
    xarray     =[]    # x values in f(x
    x=.0              # x is a starting point, say from the far left point

    g_n_minus_1    =  2*m*(E-Potential(x    ))
    g_n            =  2*m*(E-Potential(x+h  ))
    g_n_plus_1     =  2*m*(E-Potential(x+2*h))
...
# if we want to save PNG files
    ...
    # Main Results, generally it wont come below; 
    print("First Bisection Null")
    return  0,xo[0:grid],yo[0:grid-gridShift]+[0]*gridShift # zeroPadding, the last term
