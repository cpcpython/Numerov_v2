...
from Parameters_Splinev1   import gridShift
from Parameters_Splinev1   import xp
...
for i in range (len(data)): # j = exc.state v; i = grd. state v'
    ...
    print("--------------------------------- Nodes Count ------------------------------------\n\n")
    print("Check: ∫(Psi_Normalized)^2 dx ==1 ? If  vg = ",i+1,"Calculated : \t",IntPsiNorm,"E_i = ",data[i][0][0])

	# Checking every possible nodes; i => Particular energy
    totNode= 0    # total nodes
    nodesx = []   # approx location of nodes
    brakt  = 10 #  brakt number of subdivision from 1-grid
...
#            print('x,x+brakt',x,x+brakt)
            noden=brent(x,x+brakt-1,sinf)  # ...)-2  is due to ... sinf[round(b)+1])
        else:
            break
        if(noden != -1):
            totNode=totNode+1
