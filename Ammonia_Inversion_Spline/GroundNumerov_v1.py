...
#=======================================================================
# IMPORTANT PARAMETERS  : Ground State H2
#=======================================================================
...
def PlotParabolaPlus(Energies,XAll,PsiAll):
    #---------------------------------------
    # Plot Potential, Wfn,Prob. functions
    #---------------------------------------
    y=[]
    xarray=[]
    y= fittedE[0:grid] #[ Morse(x,D0,a0,Req0) for x in Xcomplete]
    xarray=Xcomplete
    ...
    fig, ax = plt.subplots()
    ax.plot(xarray[:-1], y, linewidth=2.0) # plot parabola
    ...
    for i in range(len(Energies)):
        ...
        En=Energies[i]
        ax.hlines(y=En, xmin=-xp, xmax=xp, linewidth=1, color='y') #0 xp
...
# Note it is for Zoomed graph
    ...
    ax1.plot(xarray[:-1] , y, linewidth=2.0) # plot parabola
...
def ScanHermitefunctions(InEmin,InEmax,itern,nodeMax,node):
    ...
    for i in range (itern): # upto range_32 is checked [ n=16 ] i upto 75 is checked.
        ...
        if( (ee != 0) ):
            print("return EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE",ee)
            ...
            mins=min(yyNormed) ; maxs=max(yyNormed) ; Ampyy = maxs-mins
#            if(  ( abs(yy[len(xx)-gridShift - 2]) > (abs(yy[len(xx)-gridShift - 1] ))) and \
#                   abs(yyNormed[len(xx)-gridShift-2] - yyNormed[len(xx)-gridShift+2]) <= Ampyy/20 ):
            PsiAll.append(yyNormed) # if we want Normalized Psi
#                Econverged.append(ee)
            node=node+1
            XAll=xx

            # if(abs(yyNormed[len(xx)-1]) <= 1e-10 and abs(yyNormed[len(xx)-2-gridShift])>(abs(yyNormed[len(xx)-1-gridShift]))  ):
            #     print("End Psi value ==",yy[len(xx)-1])
            #     PsiAll.append(yyNormed)
            # #node=node+1
            # #XAll=xx
#            XAll=xx
        InEmin=InEmax
        ...
        delE = 0.000015 #+delE/50 #if InEmax < -0.025 else 0.001
...
#itern      =     250        # after this iteration program exit; if you want more levels increase it
#node       =     0
#nodeMax    =     200         # Limit the node of wfn to stop the run
#Amp        =     10

Econverged,XAll,PsiAll=ScanHermitefunctions(InEmin,InEmax,itern,nodeMax,node)
