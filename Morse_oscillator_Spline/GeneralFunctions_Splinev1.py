...
def findEigenFunctions(Emin, Emax):
    ...
    # gridshift adjustement for higher quantum numbers; some trial/error needs graphically to find gshift
    for ins in range(len(gshift)):
        if(Emin > gshift[ins][0]):
            global gridShift
            gridShift=gshift[ins][1]
...
# if we want to save PNG files
    ...
    return  Emid,xo[0:grid],yo[0:grid-gridShift]+zeroP
